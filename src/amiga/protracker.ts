 ...
 * ## The tables
 *
 * The period and vibrato tables are NOT here. They belong to no one replayer —
 * AMOS's own Music extension ships the same thirty-six periods and the same
 * thirty-two sine bytes — so they live in `notes.ts`, which imports nothing and
 * is therefore reachable from `paula.ts` as well. This file re-exports them for
 * every caller that already had them from here. `protracker.test.ts` is where
 * they are checked against the shipped `AMOSPro_P61.Lib`.
 */
import { PT_PERIODS, PT_PERIODS_PER_ROW, PT_VIBRATO } from './notes'
import { PAULA_CLOCK, clampVolume, periodToHz } from './paula'
import type { AudioSink } from './host'

export { FINETUNE_STRIDE_BYTES, PT_PERIODS, PT_PERIODS_PER_ROW, PT_SINE, PT_VIBRATO } from './notes'
