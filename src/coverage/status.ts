...
export const NOTES: Record<string, string> = {
  ...
  'pt voice':
    "Routine 262 (\$60a2) sets all four per-voice bytes to \$FF first (`moveq #\$ff,d1 / move.l d1,\$a(a0)`) and then CLEARS the ones whose mask bit is clear, silencing each with `move.w #bit,\$96(a1)` on DMACON and `clr.w` on that voice's AUDxVOL. So a set bit means the music may use the voice and a clear bit both silences it and releases it -- which is what Pt Free Voice then reports on, itself eight bytes that push \$f and fall into the shared query",
  'amcaf version$':
  ...
  'red val':
    "Routines 87, 88 and 89 (\$327a onwards) are the only readers of the notation flag in the hunk, and each opens `move.w \$2d2(a2),d0 / cmp.w #\$4,d0`. At 4 they take a nibble each -- Red is `lsl.l #\$8` then a clear-and-swap, which is a shift right by eight -- and at anything else a byte each. Nothing checks that the argument fits",
  'glue colour':
  ...
  'wsave':
    "Routine 105 (\$38a2), shared by Dsave -- 'Dsave is exactly the same as Wsave in every aspect', and the token table gives both names the same routine. It checks the bank before it opens anything: `move.w -\$c(a0),d0 / andi.w #\$c,d0 / bne`, and a bank carrying either of those type bits goes to AMCAF's own requester with message 4, 'No icons- or spritesbanks allowed' -- which names exactly what the bits mark. The length written is `move.l -\$14(a0),d0` less SIXTEEN, the bank's own header, which is not part of what a program put there",
  'tool types$':
