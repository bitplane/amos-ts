...
export const NOTES: Record<string, string> = {
  ...
  'open catalog':
    "The catalog is read and parsed here, because on the machine that is locale.library's job rather than the extension's -- OpenCatalogA is behind the shim. The reader follows FORM CTLG (FVER/LANG/CSET/STRS, each STRS entry a ULONG id, a ULONG length and NUL-terminated bytes padded to a multiple of four). NOTE, and this is now the only part of the Locale port still resting on an unverified reading: no .catalog file is in the archive and none ships with the extension, so nothing here has been checked against a real Commodore-produced catalog. The tests build one byte by byte and read it back, which pins the reader against the format as understood here and no further. The search path is the plain name then CATALOGS:<language>/<name>, omitting the PROGDIR: and LOCALE: entries that need a program directory this port does not model",
  'format date$':
