...
export const NOTES: Record<string, string> = {
  ...
  'elwb open':
    "Routines 118, 119 and 120 ($213a, $214e, $217a) on intuition.library (`-$18a6(a5)`): OpenWorkBench (-$d2), WBenchToFront (-$156) and CloseWorkBench (-$4e), all three ending at routine 114 ($20c0), which is `moveq #$0,d2 / moveq #$0,d3 / tst.l d0 / beq / moveq #-$1,d3` and turns whatever the library returned into an AMOS boolean. \"AMOS provides a close workbench command, but it does not tell you whether the workbench did actually close or not.\" Close is WBenchToFront first and CloseWorkBench only if that says a screen is there, else `moveq #$ff,d0` -- which is the guide's \"Elwb close returns true if the workbench is closed when the function has finished executing, even if it didn't close it because it was already closed\". These answered the ABSENT case until there was an Intuition; they are on the real one now (src/amiga/intuition.ts), including the documented side effect of bringing the Workbench screen to the front, which is not a side effect at all but the WBenchToFront that Close and Test both open with",
  'eliconify begin':
    "Routine 124 ($21ee), 182 bytes, and the whole iconify window is in it: `tst.l $88(a2) / Rbne routine 3` (already open -> AMOS 23), OpenWorkBench (-$d2) or return 1, WBenchToFront (-$156), then `move.w (a1)+,d0 / asl.w #$3,d0 / addi.w #$50,d0` for a width of len(TITLE$) * 8 + 80, patched into the 48-byte `struct NewWindow` at $2274 along with the title, TopEdge and LeftEdge, then OpenWindow (-$cc) or return 2. The template decodes field for field: Height 11, DetailPen 0, BlockPen 1, IDCMPFlags $208 (CLOSEWINDOW | MOUSEBUTTONS), Flags $1000E (RMBTRAP | DRAGBAR | DEPTHGADGET | CLOSEGADGET), Type 1 (WBENCHSCREEN). The 11 is the number that ties the two halves of the port together -- `BorderTop = WBorTop + tf_YSize + 1` for topaz 8 -- so the window is a title bar and nothing else. NOTE: no WFLG_ACTIVATE, and the guide is explicit about the consequence: \"If you activate the window, then press the right mouse button\". The right button does not reach the window until the user has clicked on it",
  'eliconify test':
    "Routine 125 ($22a4), 120 bytes: GetMsg (-$174) on Window->UserPort ($56) in a loop, and the first message past four filters ends it -- MouseX in 0..width-1 against the width saved at $8e, MouseY in 0..9, MENUDOWN ($69) sets the latch and takes another message, and Class ($14) decides the answer: MOUSEBUTTONS ($8) is -1, anything else (CLOSEWINDOW) is 1. DEFECT: the latch at $8c is dead. `move.w` writes $8c and the test reads it with `tst.l $8c(a2)`, which covers $8e as well -- the window width, never zero after Begin -- so the `beq` is never taken and a MENUUP whose MENUDOWN happened somewhere else is accepted. Reproduced. NOTE: no ReplyMsg anywhere, so every message is leaked on the machine; the queue merely shortens here and the defect has no observable effect",
  'eliconify end':
    "Routine 126 ($231c), forty bytes: `move.l $88(a2),d0 / movea.l d0,a0 / Rbeq routine 3` -- no window is AMOS 23 -- then `move.l #$0,$88(a2)` BEFORE CloseWindow (-$48), so a failure inside the close cannot leave a stale pointer. It does not close the Workbench; the guide's procedure has the program bring AMOS back to front itself",
  'eliconify amos':
    "Routine 123 ($21d4), TWENTY-SIX bytes, and every one of them is the other three: `Rbsr 124 / tst.l d3 / bne` (1 or 2 straight back), then `Rbsr 125 / tst.l d3 / beq` round again, `bmi` keeps a -1, otherwise `moveq #$0,d3` turns Test's 1 into 0, and `Rbsr 126`. DEVIATION: the loop is AMOS's frame loop rather than a `bra` -- the 68k routine spins with the program suspended and there is nothing to spin on here until the frame that delivers the click has run. NOTE: the guide's table for THIS keyword has -1 and 0 swapped, saying \"-1 = The close window gadget was pressed. 0 = Then right mouse button was pressed\", where the code does the opposite. Its table for Eliconify Test two paragraphs later is correct and contradicts it",
  'elxpk error':
  ...
  // EasyLife slice 10: structured variables, on Easylife.Library 1.02.
  'st new':
    "Routine 263 ($37d0) into the library's `ELST_New` (LVO -48, $25a) with `move.l #$10000,d1`, MEMF_CLEAR. Looks the type up under scope 0, allocates the definition's +0 size out of a private exec pool, writes the type word and a zero flags word, and zeroes `size/4 - 1` longwords from offset 4. That clear IS the guide's initialisation table -- \"\" for strings, 0.0, False, nil, and a ranged integer at its MINIMUM, because a ranged integer is stored biased by its minimum and all-bits-zero therefore is the lowest legal value. NOTE: the pool is exec's Allocate/Deallocate over a MemHeader the library builds inside each block at `block - $e` ($2f6), so it is first-fit on an eight-byte grain with coalescing; modelled that way in elstruct.ts because reuse after a free is observable and the guide's warning about dangling pointers depends on it. DEVIATION: EXT_DATA_SLOT is 64K, so a block size above it would run into its neighbour and is capped; both sizes the guide documents ($2000 and $4000) are far below",
  'st free':
    "Routine 262 ($37b8), `ELST_Free` (LVO -42, $166). Fetches bank 12 for the block size, looks the instance's type up for its size, walks the block chain for the block containing the address and Deallocates it there. An address in no block is AMOS 23; a double free is not detected, since the second call still finds a block. \"This command does not return any memory to the system - it simply frees it for use by other structured variable instances\"",
  'st free all':
    "Routine 266 ($3834), `ELST_FreeBlocks` (LVO -72, $59c) -- the one keyword that gives the memory back, FreeMem-ing every block and zeroing `$f8`, `$f4` and `$f0`. NOTE: the mode argument is documented and dead, `move.l $f8(a6),d1` overwriting d0 before anything reads it, and `St Free All` passes 0 anyway. The block size it frees by comes from the CACHED type table at `$f0`, which is bank 12 + 12",
  'st dup':
    "Routine 267 ($384c): read the type word, `ELST_New` with d1 = 0 so NO clear, then `move.l (a2)+,(a1)+` over size/4 longwords -- the whole instance, header included. The guide: \"equivilent to (But faster than) S2=St New(St Type(S1)) : St Copy S1 To S2\", and the difference is exactly the four-byte header St Copy skips",
  'st copy':
    "Routine 268 ($387a). The DESTINATION's type word is looked up for the size and then compared with the source's, `cmp.w (a1),d3 / Rbne routine 3`, and both sides skip four bytes before `size - 4` are moved, so the destination keeps its own type and flags. NOTE: routine 3 is `moveq #$17,d0 / Rjmp L_Error`, AMOS 23 -- message 39 \"Cannot copy between structures of different types\" is in the extension's own table and NOTHING raises it. It is one of four dead entries, with 30, 37 and 38",
  'st type':
    "Routine 271 ($393a): the instance's own type word, straight out of its first two bytes. The `ELST_Lookup` under it validates only -- d3 is loaded before the call and the call cannot touch it -- so an instance whose type bank 12 does not hold raises rather than answering",
  'st len':
    "Routine 272 ($395c), TEN BYTES: `Rbsr routine 271` for the validated lookup, then `movea.l d0,a0 / move.l (a0),d3`, the definition's first longword. The guide's arithmetic reproduces it: \"adding up the lengths of all the elements, adding 4, and rounding up to the next multiple of 8\", where a string counts \"Max Len+3 rounded up to even\"",
  'st lookup':
    "Routine 294 ($3a94), `ELST_Lookup` (LVO -30, $1d8) unwrapped. Answers the ADDRESS of the definition in bank 12, not an offset: `adda.l d2,a1 / move.l a1,d0` adds the entry's offset to the bank base. DEVIATION from the guide: `d0 = (a0)+.w / dbra d0` runs count+1 times, so the table's count word holds entries MINUS ONE where the guide's own format page writes `dc.w 3` for three; all five Structs banks in the archive agree with the binary. The two bounds are the bank's own, `$10(a1)` the header's offset-to-definitions and `-$14(a1)` the AMOS node's LONG, which is the length plus sixteen -- so an offset in the final sixteen bytes passes a check meant to stop at the end",
  'st get':
    "Routines 273-276 ($3966-$397e) into the shared body 260 ($375e), `ELST_GetElement` (LVO -54, $3be) and then `move.l d0,d3 / moveq #$0,d2`. Twelve arms behind a jump table at $3f4, one per type code, and every read is zero-extended. NOTE: that `moveq #$0,d2` is why the answer is always an AMOS INTEGER -- a Real element gives back its raw longword, and a string element gives back the ADDRESS of its NUL-terminated characters, which the guide says is the point (\"it can be used for any system library calls that require a pointer to a string, or in MUI taglists\"). The subscripts arrive REVERSED, `D2=Z, D2+4=Y, D2+8=X` in the autodoc's words, because a3 is read upwards from the last thing pushed. DEFECT: the resolver's range test is `bmi` then `cmp.l d3,d2 / bcs`, which fails only when a subscript is GREATER than the bound, and both arms raise message 31 \"negative\" -- message 30 \"too high\" is never reached. Reproduced",
  'st get$':
    "Routines 277-280 ($3986-$399e) into routine 261 ($3782), which calls 260 and copies the characters out of the instance into fresh AMOS string space. \"St Get$ returns a copy of the string.\" The element's stored form is a length word, the characters, a NUL and an even pad",
  'st set':
    "Routines 282/284/286/288 ($39aa, $39d6, $3a02, $3a2e), `ELST_SetElement` (LVO -60, $46a) and a second twelve-arm table at $49a. A ranged integer is stored BIASED, `sub.l (a0)+,d6` before the bounds are checked against `(max - min)`, which is what makes an all-zero instance read back at each minimum; `bmi` is the low end and `beq / bcc` the high, so a value equal to the maximum is accepted. A typed pointer takes nil untested and otherwise reads the TARGET's type word ($4f2, the only place the library dereferences a value on its way in). Arm 11 is `bra.w $a08` and nothing else -- a sub-structure's address cannot be assigned",
  'st set str':
    "The same four routines, reached through the token entries whose value slot is a string. One body, and the descriptor's type code picks the arm, so `St Set Str` on a non-string element lands wherever that element's arm is. DEFECT: $522 copies `length + 2` bytes with a `dbra` and then writes a NUL, three bytes more than the length, into an element whose stride is `maxlen + 2` -- while the compiler's element size is `maxlen + 3` rounded up to even. A single element has the slack; an ARRAY of strings does not, and writing element N puts its terminator into element N+1's length word. Reproduced, because the port writes through the same address space",
  'st cmp':
    "Routines 290-293 ($3a7c-$3a8e) into routine 289 ($3a56), `ELST_StrCmp` (LVO -66, $548) -- compares in place, which is the point (\"much faster than using If St Get$( INSTACE, ELEMENT )= STRING$\"). The element must be type 10 or it is AMOS 23. DEVIATION: the SIGN is the guide's backwards. Both the autodoc and the command page say \"-1 if STRING$ is less than the element string\"; `cmpm.b (a0)+,(a1)+` computes destination minus source -- argument minus element -- and `blt` goes to the arm returning +1, with the length arms agreeing. Implemented as the binary has it",
  'stv':
    "Undocumented, and new in 1.10: it appears in no guide and 1.09 does not have it. The four function entries name routines 273-276, `St Get`'s own trampolines, so the read half is St Get exactly. NOTE: the write half is where the binary says something that cannot be carried over. 1.10 gives it four instruction slots, 281/283/285/287, each FOUR BYTES in front of the `St Set` body of the matching arity, which it then falls into -- and all four hold `4eac 3e2c`, `jsr $3e2c(a4)`. a4 has no value at a keyword's entry (the only three routines in 16KB that load it do so locally) and $3e2c is inside routine 300's inline message block, not code. 1.09's set family is 281-284 with no prologue, byte for byte the same forty-byte bodies, so the four bytes came in with the keyword. What is not in doubt is the fall-through, so the pair is implemented as St Get and St Set",
  'st output$':
    "Routine 270 ($38f8), and DEFECT twice over -- identically in 1.09 and 1.10, so not a 1.10 regression. $38fc is `3012`, `move.w (a2),d0`, reading the type it is about to look up out of a register nothing in the routine loads (routine 271 does the same job with `3610`, off the pointer it popped). And after the lookup $3914 puts the DEFINITION in a2, so `move.w (a2)+,d0` for the length word and `move.l (a2)+,(a1)+` for the body both read bank 12: the instance popped into a0 is never read at all. The second cannot be squared with any register assignment, because routine 269 checks the string's length against the definition's size and its first word against the instance's type and no reading of 270 produces either. The pair does not round-trip on a real Amiga; the guide's \"returns a copy of the structure instance in a string\" is implemented instead",
  'st input':
    "Routine 269 ($38b4), and its two checks ARE right and are the only two places in the whole ST block that raise a message the extension owns: the string's first word against the instance's type (message 41) and its length against the definition's size (message 40). DEFECT: $38ba is `305b`, `movea.w (a3)+,a0` -- the instance address taken as a sign-extended WORD, so the high half of the pushed longword becomes the whole pointer and AMOS's parameter stack is left two bytes out of step for the rest of the statement. Routine 270 pops the same argument with `205b`. 1.09 has the identical byte. Not reproducible: there is no a3 here to desync and a truncated address addresses nothing",
  'st save':
    "Routine 265 ($3814), `ELST_SaveTree` (LVO -96, $6e8). Twelve bytes of header -- \"ElSt\", the instance count, a zero -- then per instance its address at save time and its bytes, no length, because the type word gives it. The graph comes from `ELST_TreeScan` ($5f8), breadth-first with a visited bit at the instance's flags word. NOTE: the library takes DOSBase from `$2b8(a5)`, AMOS's own workspace register, which is still live because the extension calls in with `movea.l $1e8(a5),a6` and never touches a5. NOTE: the Open() failure exit sets d0 = $5e and routine 299 hands non-negative d0 to L_Error, so the AMOS error raised is 94, \"Next without For in animation string\" -- plainly not the message meant, and raised unchanged here because the alternative is inventing one",
  'st load':
  ...
  'eltest':
    "1.09 ONLY --- 1.0 does not have it, 1.44 does not, and 1.10 dropped it and put `Stv` at the same id \\$e4e. In 1.09 it is the LAST entry of a 220-entry table, spec `V00,0`, and both routines are EIGHT BYTES: 1.09's routine 255 (\\$372a) is `moveq #\\$1,d0 / lea \\$c(a3),a3 / rts` and 1.09's routine 256 (\\$3732) the same with `moveq #\\$0,d0`. The one register they set is the one that tells the two entry points apart, which is what the name says it is: the author's probe for checking AMOS's V-form dispatch reaches the slot it should. In none of the three guides. The instruction half pops three longwords, which is right for two arguments and a value, so it is a clean no-op and is implemented as one --- the same call as 1.44's Elzqzqzq. DEVIATION: the function half pops three longwords too, one more than its two arguments, and sets d0 where every other function here returns in d3 with a type in d2, so `=Eltest(a,b)` answers whatever d3 held from the last thing that set it. Zero is answered instead, typed as the integer the spec's leading `0` asks for; an undefined register is not a value this port can reproduce",
