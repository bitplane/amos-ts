...
export const NOTES: Record<string, string> = {
  ...
  'td redraw':
    'The model is complete and the drawing is not. Everything the engine does before a pixel is touched is here and tested: the three screen checks at $211418, the frame stamp at a4+$1902 that never wraps to zero, the walk of the live instance list, the attitude matrix from $213df8, the vertex transform from $2108a2, the view transform and perspective divide from $2101c8, and the near and far limits — sixteen world units, and 32767 where divs.w\'s word-sized divisor wraps. tdRedrawFaces hands back every live instance\'s faces in screen coordinates, in sixteenths of a pixel. What is missing is the fill, and the reason is the surface format: a .3DS is not a colour but a "surface detail" with its own anchor points, and dice\'s six run from 318 to 1848 bytes of nested geometry — the pips on the faces of a die. Until that is decoded there is nothing honest to fill a polygon with, so a program that calls Td Redraw gets its screen validated and its frame advanced and sees nothing drawn',
  'td load':
