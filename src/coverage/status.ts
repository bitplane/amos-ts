...
export const NOTES: Record<string, string> = {
  ...
  'td redraw':
    'The model is complete and the drawing is not. Everything the engine does before a pixel is touched is here and tested: the three screen checks at $211418, the frame stamp at a4+$1902 that never wraps to zero, the walk of the live instance list, the attitude matrix from $213df8, the vertex transform from $2108a2, the view transform and perspective divide from $2101c8, and the near and far limits — sixteen world units, and 32767 where divs.w\'s word-sized divisor wraps. The .3DS surfaces are decoded too, so every face arrives with its own patches already placed and penned. What is missing is the face colour: a face is filled with a dither pair rather than a pen, Td Set Colour writes two of the bottom four pens through $38(part) out of the sixteen pairs at a4+$54, and where a part\'s pair points before any Td Set Colour has not been located. So a program that calls Td Redraw gets its screen validated, its frame advanced and its geometry computed, and sees nothing drawn',
  'td load':
