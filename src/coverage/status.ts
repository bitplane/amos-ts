...
export const NOTES: Record<string, string> = {
  ...
  'td redraw':
    'The model is complete and the drawing is not. Everything the engine does before a pixel is touched is here and tested: the three screen checks at $211418, the frame stamp at a4+$1902 that never wraps to zero, the walk of the live instance list, the attitude matrix from $213df8, the vertex transform from $2108a2, the view transform and perspective divide from $2101c8, and the near and far limits — sixteen world units, and 32767 where divs.w\'s word-sized divisor wraps. The .3DS surfaces are decoded, so every face arrives with its patches placed and penned, and every face carries the dither pair its block is drawn in — the two bytes per block at the object\'s +$3a section, which $2109c0 copies into the array Td Set Colour later writes through. What is left is the rasteriser: the EOR-edge scanline fill at $2105e0, the two-pen dither, and the buffer Td Cls clears. So a program that calls Td Redraw gets its screen validated, its frame advanced and every polygon computed, and nothing reaches the bitmap',
  'td load':
