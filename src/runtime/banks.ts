 ...
 *
 *     Bnk.ResIco (+Lib.s:8116)  moveq #(1<<Bnk_BitIcon)+(1<<Bnk_BitData),d2
 *     Bnk.ResBob (+Lib.s:8124)  moveq #(1<<Bnk_BitBob)+(1<<Bnk_BitData),d2
 *
 * Both carry Bnk_BitData, and that answers a question the old comment on
 * `Erase Temp` guessed at: Bnk.EffTemp (+Lib.s:8030) tests `btst
 ...
 * FnLength (+Lib.s:2462) computes the byte length and then, for a Bob or Icon
