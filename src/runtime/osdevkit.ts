...
export function makeOsDevKitFunctions(rt: Runtime): Record<string, Func> {
  ...
  return {
    ...
    '_dt do'(_, a) {
      ...
      if (method === DTM.Trigger) {
        const triggerFunction = structRead(rt, message + 8, 4, false)
        const data = structRead(rt, message + 12, 4, false)
    ...
    '_prg dir$'() { return VS(rt.launchDirectory) },
    '_prg name$'() { return VS(rt.launchFileName) },
