...
const AUDITED = new Map<string, { status: OsBackendStatus; reason: string }>([
  ...
  ['_dt create', { status: 'partial', reason: 'shared descriptor-backed datatype objects and attributes exist; datatype-class decoding is limited to runtime-supported media' }],
  ...
  ['_dt what attrs', { status: 'partial', reason: 'known attributes write through caller result pointers; class-computed attributes remain incomplete' }],
  ...
  ['_dt what methods', { status: 'partial', reason: 'a stable native method list is exposed, limited to the shared generic object surface' }],
  ['_dt what triggers', { status: 'partial', reason: 'a stable native trigger list is exposed, without format-specific trigger methods' }],
  ['_dt do', { status: 'partial', reason: 'method dispatch validates shared objects; arbitrary native messages and class-specific methods remain incomplete' }],
  ...
  ['_asl alloc', { status: 'partial', reason: 'all three requester types allocate caller-visible native prefixes; allocation TagItems are not yet applied' }],
  ['_asl do', { status: 'partial', reason: 'file, font and screen-mode dialogs share the runtime modal backend and update public fields; request-time TagItems are not yet applied' }],
