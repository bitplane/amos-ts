...
const AUDITED = new Map<string, { status: OsBackendStatus; reason: string }>([
  ...
  ['_dt obtain', { status: 'faithful', reason: 'V39 IFF/ASCII/binary classification, priority-order masks, MacPaint validation and file/IFF-stream sources share the installed descriptors' }],
  ['_dt release', { status: 'faithful', reason: 'shared immutable descriptors make ReleaseDataType observably a no-op' }],
  ['_dt create', { status: 'partial', reason: 'shared BOOPSI objects decode every installed picture and sound class, FTXT and AmigaGuide; some optional class attributes remain incomplete' }],
  ...
  ['_dt do', { status: 'partial', reason: 'all advertised media methods dispatch, including print, selection, write, sound play and AmigaGuide navigation; print options and AmigaGuide SYSTEM/ARexx commands remain incomplete' }],
...
auditMany('faithful', 'the high-level string-array conversions and menu flag operations match the native allocation and flag semantics', [
  ...
  '_gt gadgets bank', '_gt menus bank',
...
auditMany('partial', 'the selected Screen-ID routes drawing, text, scrolling, patterned area-fill, flood and AMOS Bob images through the shared RastPort; scratch raster allocation follows the worker, while raw temporary AreaInfo/TmpRas pointers are intentionally not exposed', [
  '_scr id clip', '_scr id fill ellipse', '_scr id scroll', '_scr id put bob',
])
...
auditMany('faithful', 'workers 3025-3028 and 3035, 3036 and 3038 compose the already-faithful native RastPort Move, Draw, DrawEllipse, Text, RectFill and Flood operations exactly', [
  '_scr id rect', '_scr id line to', '_scr id line', '_scr id ellipse', '_scr id paint', '_scr id bar', '_scr id text',
])
