...
describe.skipIf(!present)('OS DevKit backend inventory', () => {
  ...
  it('classifies every high-level screen-ID wrapper and its indirect worker', () => {
    ...
    expect(idRows.filter((row) => row.status === 'faithful').map((row) => row.name).sort()).toEqual([
      ...
      '_scr id height', '_scr id hide', '_scr id in use', '_scr id mode', '_scr id move', '_scr id offset',
      '_scr id rport', '_scr id show', '_scr id use', '_scr id vport', '_scr id width',
    ])
    expect(idRows.filter((row) => row.status === 'partial')).toHaveLength(40)
