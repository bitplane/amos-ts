...
function area(name: string): string {
  ...
  // InGet/InPut +Lib.s:5265) — files, not the object-bank Get/Put family
  ...
  const summary = mdTable(
    ...
    [
      ...areas.map(([name, rs]) => [
        name,
        String(rs.length),
        String(count(rs, 'faithful')),
        String(count(rs, 'approximated')),
        String(count(rs, 'missing')),
        pct(rs),
      ]),
      [
        '**total**',
        String(rows.length),
        String(count(rows, 'faithful')),
        String(count(rows, 'approximated')),
        String(count(rows, 'missing')),
        pct(rows),
      ],
    ],
  )
...
/**
 * A comma-separated list broken at 96 columns, continuation lines indented to
 * sit under the first item.
 *
 * AMCAF's faithful list is 280 keywords and was one line of 17,657 characters.
 * Nothing renders that usefully — an editor either wraps it into a paragraph
 * with no structure or scrolls it sideways for a screen and a half — and in a
 * `git diff` a single keyword moving between two of these lines reports the
 * whole area as changed. Markdown treats a wrapped list item as one item, so
 * the rendered page is identical and only the source becomes readable.
 */
function wrap(items: readonly string[], width = 96, indent = '  ', lead = 0): string {
  const lines: string[] = []
  let line = ''
  let budget = width - lead
  for (const [i, item] of items.entries()) {
    const piece = i === items.length - 1 ? item : `${item},`
    if (line === '') line = piece
    else if (`${line} ${piece}`.length <= budget) line += ` ${piece}`
    else {
      lines.push(line)
      line = piece
