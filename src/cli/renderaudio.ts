...
export type Engine = ModFormat | 'track' | 'medext' | 'sid'
...
if (process.argv[1]?.endsWith('renderaudio.ts')) {
  ...
  if (!file || !out) {
    console.error(
      'usage: renderaudio <module> <out.wav> [--engine mod|p61|thx|med|omed|sfx|fc14|fc13|digi|smon|s3m|sid|track|medext]',
    )
