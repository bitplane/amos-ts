...
export type Engine = 'mod' | 'p61' | 'thx' | 'med' | 'sfx' | 'fc14' | 'fc13' | 'digi' | 'track' | 'medext'
...
if (process.argv[1]?.endsWith('renderaudio.ts')) {
  ...
  if (!file || !out) {
    console.error('usage: renderaudio <module> <out.wav> [--engine mod|p61|thx|med|sfx|fc14|fc13|digi|track|medext]')
