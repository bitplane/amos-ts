 ...
 * arena is not per-program: `ResDir` (+Verif.s:4040) carves the direct slots
 * off the top of the SAME `TabBas`.
 *
 * `DirFlag` and `Direct` are different words. `DirFlag` tells the verifier the
 * buffer holds a bare token stream with no line header (`SsTest` +Verif.s:265);
 ...
 * that verifies it: `VerSStack` (+Verif.s:816), `VerSBu` (:834), `V1_Proc`
 ...
 *
 * The other four are token KINDS rather than keywords. `VerRem` (+Verif.s:773)
