...
| `osbackend.ts` | inspect the OS DevKit operation verdicts used by `KEYWORDS.md` and add direct binary routine/library/LVO traces; `--json` emits every row and reason but is not a second coverage-list generator |
