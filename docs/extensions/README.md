...
88 extensions are registered, 5 of them stock.
...
| `gui-1.5b`             | GUI                                   | disassembly |       48 |      24 | states 24     |
...
| `tft-0.7`              | TFT                                   | disassembly |       28 |       — | states 25     |
...
1994), scanned with `libcat` and written up by hand. Their
...
sets calls the two different. `libcat` compares named `(id,name)` pairs for
exactly this reason, so an unnamed padding entry does not manufacture a gap.
...
that was arrived at independently**: of the ten slots in the identification
sample used at that stage,
...
- **slot 14**, where he lists The Game Extension, and puts Intuition at 25 with
  the note "Slot 14 originally?". Every sampled program using Intuition had it
  at 14, which answered his question from the programs themselves.
...
The registry is the inventory and [../../KEYWORDS.md](../../KEYWORDS.md) is the
generated coverage view. Discovery is no longer the planning loop: the
collected corpus covers the large surviving AMOS collections, and the token
...
a complete keyword classification, including explicit approximations and n/a
entries.

## Adding material

The program corpus and extension registry answer different questions. A program
records slot numbers and token ids; a library carries the complete token table.
When genuinely new material appears:

1. Preserve it in `amos-files` with its source and licence provenance.
2. Run `libcat` over any `.Lib` files. It reports exact registered tables,
   variants, renumbered releases and new tables; `--gap`, `--keywords` and
   `--json` provide the detailed views.
3. Compare program token use one program at a time with the identification
   functions in `src/ext/identify.ts`. Never merge one slot across unrelated
   machines: a slot number is configuration, not identity.
4. Add or update a manifest only after checking the library's own name,
   version, token table, provenance and binary.
5. Regenerate with `npm run gentables` and `npm run gendocs`.

A scanned token table is evidence that a program used that table. It does not
establish the product's author, release, licence, recommended slot or behavior.
Those remain manual evidence work.

The old whole-corpus wanted-list and popularity surveys were removed after their
discoveries had been recorded. They selected the early extension work, but
keeping their snapshots beside the live registry made completed ports look
unfinished and implied that acquisition was still the project's main task.
