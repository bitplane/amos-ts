...
|---|---|---|
| OS DevKit 1.61 (`os-devkit-1.61`) | 1047 | a wrapper over most of AmigaOS. The old claim that 932 already had a back-end was not reproducible: its named missing families summed to 81 rather than the implied 115, and “library exists” had been counted as “operation exists.” `src/cli/osbackend.ts` now accounts for every named entry without that leap: 741 are candidates in a modelled family, 57 are in five known-missing families, and 249 still require operation-level review. These are audit categories, not implementation coverage. Routine-level tracing currently proves direct AmigaOS calls for 389 keywords and reports the library and LVO for each; 13 detected calls still have untraced bases. The largest candidate groups are intuition 330, graphics 130, gadtools 120, exec 60 and dos 54; the known missing groups are commodities 24, iffparse 16, workbench 11, preferences 3 and amigaguide 3. Run with `--json` for all 1,047 rows |
