...
|---|---|---|
| OS DevKit 1.61 (`os-devkit-1.61`) | 1047 | a wrapper over most of AmigaOS. The old claim that 932 already had a back-end was not reproducible because “library family exists” had been counted as “operation exists.” `src/cli/osbackend.ts` now requires an operation-level verdict. The first completed family, DataTypes, has 2 faithful keywords, 1 partial and 11 missing; with the five already-known missing families this makes the current whole-extension count 2 faithful, 1 partial, 68 missing and 976 still under review. Routine tracing identifies every detected direct call across 18 libraries. Run with `--json` for the evidence and reason attached to all 1,047 rows |
