...
Registered but still lacking callable extension handlers. The OS DevKit row is
different from the other two: much of its lower-level AmigaOS behavior is
already modelled, but that backend inventory is not itself an implementation
of the AMOS extension keywords.

| extension | missing | what it is waiting on |
|---|---|---|
| OS DevKit 1.61 (`os-devkit-1.61`) | 1047 | a wrapper over most of AmigaOS. **The binary/backend audit is complete, but the extension port has not started: all 1,047 AMOS keywords still lack handlers and therefore remain missing in `KEYWORDS.md`.** The audit describes reusable lower-level capability, not callable keyword coverage. The old claim that 932 already had a back-end was not reproducible because “library family exists” had been counted as “operation exists.” `src/cli/osbackend.ts` now requires an operation-level verdict. All families, including Intuition, are audited, giving 553 faithful, 388 partial and 106 missing backend verdicts, with none still under review. The high-level `_gt` bank workflow is partial as a group: its machine primitives exist, but its integrated OS DevKit layer does not. StonePlayer's library binary is absent from both its archive and the held external library set; only the `_fx` Samples-bank operations overlap the existing backend. `CacheControl` preserves its exact masked control word and return value, but remains partial until a 68k execution cache exists for those flags to affect; `SetChipRev` likewise records the request against a machine fixed as AGA. Exec V39 pool lifetime and allocation are implemented, with native puddle placement still absent. DOS FilePart/PathPart boundary rules, lock-derived current directories and no-requester existence checks are modelled; raw C buffers, BPTR lock identity, shared/exclusive ownership and full `AddPart` edge semantics are not. Despite its guide calling `_dos l open` a lock-to-file-handle conversion, worker 1828 calls `ParentDir`, and the audit follows the binary. Managed AMOS file channels cover sequential/random reads, writes, seeking, buffering, EOF/length and names, but OS DevKit's raw DOS handles and caller buffers do not; process-wide `IoErr`/`SetIoErr` and `ReportEvent` are absent, while `Fault` has only the error-string subset. Hunk validation and host process launch cover the semantic LoadSeg/CreateNewProc path without native segment lists; VFS subscriptions detect changes but are not wired to native NotifyRequest signal/message delivery. NotifyRequest user data is an exact field; the launch name/current directory exist, but the original startup WBArg pair is not retained independently. The private `_chn` header layout and one-based list operations are implemented; resize safely preserves the common prefix instead of reproducing the worker's max(old,new) buffer overrun. Dots arrays use exact signed-word pairs and BooleanInfo preserves its packed word/unaligned-long widths. V39 pen allocation, best/coerced mode selection and CalcIVG have semantic palette/display geometry but lack arbitrary native ColorMaps, tag policy and driver structures. The internal 32-class resource tracker now reproduces deduplicating Set, direct Add, lookup and gap-closing removal. The saved d0-d7/a0-a7 frame and truncated private AMOS name are represented, but arbitrary native calls, the A3 evaluation-stack address and internal channel-table pointer are unavailable; `_low init` is an exact null-function token. Managed BOOPSI Dispose/GetAttr are exact; construction and arbitrary tags/messages remain partial, and the binary's custom file-image class is absent. Alert is absent; refresh, overscan, choice requests, VPrintf, GT reserve aliases and the direct-register Bob blitter have narrower semantic backends. The complete 44-byte Gadget field surface is represented exactly; lifecycle calls remain partial where they require arbitrary native Gadget/Window chains and refresh drawing. The retained NewScreen definition and direct Screen scalar/pointer fields are exact; global front/active and embedded-structure queries remain partial because native interior addresses are not stable. Managed screens open, close, reorder and move, and Workbench/public-screen and DrawInfo models exist; arbitrary native NewScreen/Screen/tag pointers, title-bar and DisplayBeep presentation, tracked native DrawInfo allocations and the complete process-wide public-screen registry do not. The high-level screen-ID table delegates to those same lifecycle, palette and RastPort workers; only its V1/V2 default pen arrays are exact local state, while the integrated numbered-screen records, native pointer bridge, clipping ownership and input.device mouse warp remain absent. The complete NewWindow definition and direct public Window field surface are represented; lifecycle, arbitrary native pointers, global active/ViewPort identity, shared UserPorts and blocking message ownership remain partial. The final high-level Window-ID record, selected-window state, copied event fields, geometry queries and exact RastPort delegates are represented; native event queues, menu-chain traversal, pointer/mouse control and Area/Bob ownership remain partial. `_wnd what pointer` itself tests stale `a0` before loading the supplied Window, so its null-error behavior cannot be reproduced by an ordinary call. DOS DateStamp arithmetic is exact, while DateToStr localization and Preferences formats are not. Global ENV: variables are partial; local variables, FindVar pointers and ReadArgs template/result state are absent. Generic library calls remain behind arbitrary 68k LVO execution, while Topaz and tag-list objects lack raw native pointer layouts. DoubleClick uses the fixed Workbench default rather than user Preferences. Border, Image, IntuiText, TextAttr, RasInfo, View, ViewPort, BitMap, ColorMap, SimpleSprite, TmpRas, RastPort, message, Exec List/Node, port/signal and Interrupt records are complete; arbitrary interrupt code, old-style Requester records, global arena accounting, blocking waits, scheduling, rendering and native-pointer guards remain incomplete. EasyRequest uses the host requester path. Per-window pointer definitions are retained but their sprites are not rendered; mouse-report flags and IntuitionBase lock pairing are exact. The installed PAL display-mode lookup is exact, while full monitor-driver data records and staged display rebuilds are not. BitMap allocation lacks interleaved and friend layouts; `_bm set plane` safely omits the binary's negative-index memory corruption. ColorMap storage and direct RGB conversion are exact; RGB load/set through arbitrary native ViewPort and raw table pointers remains partial. View/ViewPort initialization is exact, while the remaining Copper lifecycle calls lack arbitrary native View graph integration. SimpleSprite fields, TmpRas and RastPort fields/drawing primitives are exact; clear geometry, PolyDraw arrays and ScrollRaster exposure remain partial. Classic and V39 sprite allocation/display calls lack arbitrary native structure and tag-list ownership. Blitter ownership and waits are exact no-ops in the synchronous backend; raw addresses, arbitrary masks/patterns/RastPorts and the full minterm surface remain partial. RastPort font selection and soft style are exact; resident-list ownership and arbitrary native TextFont/TextAttr calls remain partial despite disk-font parsing support. Area-fill geometry exists, but native AreaInfo/TmpRas and raw raster ownership are not wired into it. BitMapScale/ScalerDiv lack native DDA argument and rounding behavior; V39 RP tag gets lack raw output pointers. `_mem abs alloc` is a misleading name for an exact alias of `_mem alloc`, not an AllocAbs call; `_sys revision` likewise reads ExecBase `SoftVer`, not `lib_Revision`. `_view set` has a shipped defect: its modes long overwrites Y/X and leaves `v_Modes` unchanged. `_vp set body` writes its priority as a word across the priority and extended-modes bytes, `_vp what width` reads absolute `$18`, and `_vp what y` dereferences stale `d0`; the backend reproduces all four defects. Overload continuations are grouped with their named keyword, including 22 formerly uncited routines. Routine tracing identifies every detected direct call across 18 libraries. Run with `--json` for the evidence and reason attached to all 1,047 rows |
