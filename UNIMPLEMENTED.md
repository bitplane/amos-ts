...
|---|---|---|
| OS DevKit 1.61 (`os-devkit-1.61`) | 1047 | a wrapper over most of AmigaOS. The old claim that 932 already had a back-end was not reproducible because “library family exists” had been counted as “operation exists.” `src/cli/osbackend.ts` now requires an operation-level verdict. DataTypes, LowLevel, Locale, ASL, Icon, Layers, GadTools, Utility-adjacent, StonePlayer, CPU-memory, cache, chipset, reset and local runtime/native-structure helpers are audited, giving a current whole-extension count of 286 faithful, 143 partial, 95 missing and 523 still under review. The high-level `_gt` bank workflow is partial as a group: its machine primitives exist, but its integrated OS DevKit layer does not. StonePlayer's library binary is absent from both its archive and the held external library set; only the `_fx` Samples-bank operations overlap the existing backend. `CacheControl` preserves its exact masked control word and return value, but remains partial until a 68k execution cache exists for those flags to affect; `SetChipRev` likewise records the request against a machine fixed as AGA. Exec V39 pool lifetime and allocation are implemented, with native puddle placement still absent; DOS path helpers lack the native fixed-buffer and complete `AddPart` edge semantics. Generic library calls remain behind arbitrary 68k LVO execution, while Topaz and tag-list objects lack raw native pointer layouts. DoubleClick uses the fixed Workbench default rather than user Preferences. Border, Image, IntuiText, TextAttr, RasInfo, View, ViewPort, BitMap, message, Exec List/Node, port/signal and Interrupt records are complete; arbitrary interrupt code, old-style Requester records, global arena accounting, blocking waits, scheduling, rendering and native-pointer guards remain incomplete. EasyRequest uses the host requester path. Per-window pointer definitions are retained but their sprites are not rendered; mouse-report flags and IntuitionBase lock pairing are exact. The installed PAL display-mode lookup is exact, while full monitor-driver data records and staged display rebuilds are not. BitMap allocation lacks interleaved and friend layouts; `_bm set plane` safely omits the binary's negative-index memory corruption. `_mem abs alloc` is a misleading name for an exact alias of `_mem alloc`, not an AllocAbs call; `_sys revision` likewise reads ExecBase `SoftVer`, not `lib_Revision`. `_view set` has a shipped defect: its modes long overwrites Y/X and leaves `v_Modes` unchanged. `_vp set body` writes its priority as a word across the priority and extended-modes bytes, `_vp what width` reads absolute `$18`, and `_vp what y` dereferences stale `d0`; the backend reproduces all four defects. Routine tracing identifies every detected direct call across 18 libraries. Run with `--json` for the evidence and reason attached to all 1,047 rows |
