...
|---|---|---|
| OS DevKit 1.61 (`os-devkit-1.61`) | 1047 | a wrapper over most of AmigaOS. The old claim that 932 already had a back-end was not reproducible because “library family exists” had been counted as “operation exists.” `src/cli/osbackend.ts` now requires an operation-level verdict. DataTypes, LowLevel, Locale, ASL, Icon, Layers, GadTools, Utility-adjacent, StonePlayer, CPU-memory, cache, chipset, reset and local string/structure/vector helpers are audited, giving a current whole-extension count of 89 faithful, 101 partial, 90 missing and 767 still under review. The high-level `_gt` bank workflow is partial as a group: its machine primitives exist, but its integrated OS DevKit layer does not. StonePlayer's library binary is absent from both its archive and the held external library set; only the `_fx` Samples-bank operations overlap the existing backend. `CacheControl` preserves its exact masked control word and return value, but remains partial until a 68k execution cache exists for those flags to affect; `SetChipRev` likewise records the request against a machine fixed as AGA. Routine tracing identifies every detected direct call across 18 libraries. Run with `--json` for the evidence and reason attached to all 1,047 rows |
